...
def start_initialization(stakan_0, stakan_1, stakan_2, stakan_3, stakan_4, stakan_5, stakan_6, stakan_7, stakan_8,
                         stakan_9, stakan_10, list_bumag, win_pl, win_nr, hod):  # основная функция игры
    global kolvo_palochek  # глобалим палочки:)
    kolvo_palochek = 11

    stakans = [stakan_0, stakan_1, stakan_2, stakan_3, stakan_4, stakan_5, stakan_6, stakan_7, stakan_8, stakan_9,
               stakan_10]  # список стаканов, внутри которых списки с бумажками
    ...
    def game_1_round():  # первый ход(определяется рандомом) для дальнейшего определения ходов
        ...
        if hod == 0:  # нулевой ход - ход игрока
            hod_igroka = int(
                input("Сколько палочек вы хотите вытащить?(от 1 до 2) отсталось " + str(kolvo_palochek) + " штук - "))
            while hod_igroka > 2:
                print(
                    "ээээ, это не одна и не две, давай переигрывай, для тупых что ли написал?")  # не ну это рил для тупых кто читать не умеет
                hod_igroka = int(input(
                    "Сколько палочек вы хотите вытащить?(от 1 до 2) отсталось " + str(kolvo_palochek) + " штук - "))
        ...
        if hod == 1:  # первичный ход - ход нейронки
            ...
            print("Нейронка вытащила " + str(hod_ner) + " палочку, ну или палочки. Их осталось " + str(
                kolvo_palochek) + " штук")
            hod = 0
    ...
    def nexts_rounds():  # функция для послдующих мини-раундов после первого определения хода
        ...
        if hod == 0:  # определяем чей сейчас ход
            ...
            while hod_igroka > 2:
                print(
                    "ээээ, это не одна и не две, давай переигрывай, для тупых что ли написал?")  # вставочка для тупых:)
                hod_igroka = int(input(
                    "Сколько палочек вы хотите вытащить?(от 1 до 2) отсталось " + str(kolvo_palochek) + " штук - "))
            ...
            if kolvo_palochek < 1:
                if hod == 0:
                    print(
                        "В этом раунде победил игрок")  # также определение того, кто победил, а кто проиграл, но только для пользователя
                if hod == 1:
                ...
                for i in list_bumag_2:
                    for q in list_bumag_1:
                        ...
                        for d in stakans[in_n]:
                            if d == delete_cards:
                                stakans[in_n].remove(
                                    d)  # и щас назрел очень хороший вопрос... а где это вызывать вообще?..
        ...
        while kolvo_palochek != 0:
            if kolvo_palochek == 11:  # определение первого хода
                ...
                if kolvo_palochek <= 1:
                    ...
                    check_you = int(input(
                        "Вы хотите продолжить обучать нейросеть путем дальнейших игр? (0 - нет, будет выведено содержимое стаканов. 1 - да, игра продолжится, содержимое стаканов отображаться не будет)"))
                    if check_you == 0:  # если пользователю надоело учить нейросеть, то он выходит из программы и видит результат обучения
                        ...
                        print(win_pl, " -победы игрока ", win_nr, " - победы  игрока ", stakans,
                              " - содержимое всех стаканок, пасиб за игру:)")
                        break
...
start_initialization(stakan_0, stakan_1, stakan_2, stakan_3, stakan_4, stakan_5, stakan_6, stakan_7, stakan_8, stakan_9,
                     stakan_10, list_bumag_1, win_pl, win_nr, hod)  # начало игры
...
# OK. За работу 4, плюс за старание дополнительный балл. Очень круто сделано, с душой. Не докрутил только обучалку.
# Надо бы починить. Предлагаю использовать вычитание списков, чтобы выбрасывать из стаканов содержимое list_bumag, например.
# Вот годная ссыл очка в помощь: https://ru.stackoverflow.com/questions/642636/Быстрое-вычитание-списков